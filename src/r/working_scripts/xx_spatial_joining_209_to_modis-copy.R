...
foreach(i = 1:length(years))%dopar%{
  ...
  events_209 <- read_csv(events_table_filename, guess_max = 50000)%>%
  ...
  events_209 <- events_209  %>%
    ...
    st_buffer(10000) %>%
    mutate(modis_id = raster::extract(events_modis_r, ., 
...
for(i in 1:length(result_gpkgs)){
  ...
  results_df[i,2] <- read_csv(events_table_filename, guess_max = 50000)%>%
    filter(is.na(POO_LATITUDE)==FALSE,
           START_YEAR==years[i]) %>%
    nrow
...
st_write(dd, "matches_209_data.gpkg", delete_dsn = TRUE)
...
ggplot(ddd, aes(y=fsr, x=CAUSE)) + geom_boxplot(outlier.shape = NA) + ylim(c(0,35))
