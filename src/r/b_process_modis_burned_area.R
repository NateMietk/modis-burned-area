...
tiles <- MODISr::get_tiles(states)

# Download all HDF files from 2001-2017 for the CONUS
MODISr::download_tiles(tiles, out_dir = hdf_months)

# Convert the raw HDF to GeoTifs
MODISr::convert_hdf_tif(tiles, in_dir = hdf_months, out_dir = tif_months, n_cores = parallel::detectCores(), layer_names = "BurnDate")

# Create yearly raster based on the max burned date of each month
MODISr::aggreate_to_yearly(tiles, n_cores = parallel::detectCores(), year_range = '2001:2017', 
                   in_dir = tif_months = tif_year, out_dir)

# Mosaic the yearly tiles to yearly CONUS rasters
MODISr::mosaic_tiles(year_range = '2001:2017', n_cores = parallel::detectCores()/2,  in_dir = tif_months, out_dir = yearly_composites, 
