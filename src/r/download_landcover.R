...
url1 <- "https://e4ftl01.cr.usgs.gov/MOTA/MCD12Q1.006/2001.01.01/"
u_p <- "admahood:Ulmac323"
...
#whole world
tiles <- unique(dir_listing1$tiles)
#USA
...
foreach (y = 1:length(years))%dopar%{
  dir.create(paste0("data/MCD12Q1/", years[y]), showWarnings = F,recursive = T)
  ...
  for(i in dir_listing1$X1){
    output_file_name <- file.path("data/MCD12Q1",years[y],i)
  ...
  system(paste(
    "aws s3 sync",
    file.path("data/MCD12Q1",years[y]),
    file.path("s3://earthlab-natem/modis-burned-area/input/landcover",years[y])
  ))
  system(paste("rm",file.path("data/MCD12Q1",years[y])))
}
